...
func init() {
	...
	execCmd.Flags().DurationVarP(&sessionTTL, "session-ttl", "t", time.Hour, "Expiration time for aad role session")
...
func execRun(cmd *cobra.Command, args []string) error {
	...
	env.Set("AWS_AAD_PROFILE", profile)
	env.Set("AWS_AAD_ASSUMED_ROLE_ARN", roleARN)
	env.Set("AWS_AAD_ASSUMED_ROLE", role)
	...
	env.Set("AWS_AAD_SESSION_EXPIRATION", fmt.Sprintf("%d", p.GetExpiration().Unix()))
