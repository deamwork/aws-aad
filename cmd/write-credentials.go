...
var writeToCredentialsCmd = &cobra.Command{
	...
	Example:   "aws-aad write-to-credentials test ~/.aws/credentials",
	ValidArgs: listProfileNames(mustListProfiles()),
}

func init() {
	RootCmd.AddCommand(writeToCredentialsCmd)
	writeToCredentialsCmd.Flags().DurationVarP(&sessionTTL, "session-ttl", "t", time.Hour, "Expiration time for aad role session")
