...
func ParseSAML(body []byte, resp *SAMLAssertion, tenant string) (err error) {
	doc := parseSAML2(string(body), tenant)
...
func parseSAML2(body, tenant string) []byte {
	template := `<samlp:Response ID="_%s" Version="2.0" IssueInstant="%s"
            ...
            <Issuer xmlns="urn:oasis:names:tc:SAML:2.0:assertion">https://sts.windows.net/%s/</Issuer>
            <samlp:Status><samlp:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:Success"/></samlp:Status>
            %s
        </samlp:Response>`
	return []byte(fmt.Sprintf(template, uuid.New(), time.Now().UTC().Format(time.RFC3339), tenant, body))
}
